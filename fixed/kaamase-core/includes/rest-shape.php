...
if ( ! function_exists( 'kaamase_shape_employer' ) ) {
	/**
	 * An employer, as the directory shows them.
	 *
	 * There is no phone number in this array, at either level of detail,
	 * and that is not an oversight. Contact runs through
	 * kaamase_can_contact() one profile at a time and is counted against
	 * the daily reveal cap. A list endpoint that carried numbers would
	 * hand out in one request what that cap exists to ration.
	 *
	 * Every value below is read through kaamase_field(), so the privacy
	 * layer applies here exactly as it does on the website. Nothing is
	 * read from post meta directly.
	 *
	 * @since 1.4.1
	 * @param WP_Post|int $post The employer.
	...
	function kaamase_shape_employer( $post, $full = false ) {
		...
		$out = array(
			...
			'employer_type'  => (string) kaamase_field( $id, 'employer_type', 'individual' ),
		...
		if ( $full ) {
			...
			/*
			 * The contact person's name is on the profile page already
			 * and is what somebody asks for when they ring. It is not a
			 * private field; the number they would ring is.
			 */
			$out['contact_name'] = (string) kaamase_field( $id, 'contact_name' );
		}

		/**
		 * Filter an employer as the API returns them.
		 *
		 * @since 1.4.1
		 * @param array   $out  Shaped employer.
		 * @param WP_Post $post The employer.
